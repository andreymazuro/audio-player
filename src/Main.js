...
import {getInfo, checked, check, transform} from './storage'
...
class MainPage extends Component{
  ...
  nextSong = (index) => {
    this.setState({current: this.state.topSongs[index + 1],
  ...
  cellClicked = (rowNumber, columnId,e) => {
    if (columnId !== 1) {
      this.setState({
        current:this.state.topSongs[rowNumber],
  ...
  render(){
    ...
    const current = transform(this.state.current)
        ...
        <div>
            ...
            <TableHeader>
              ...
              <TableRow key={index}>
                  ...
                  {(current.trackId === item.id.attributes['im:id'])? (
